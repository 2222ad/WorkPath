...
//    public GreedySolver(int weekNum, double m2, double m3, double m4, double m5, double m6, double m7, double F1, double F2, List<CurSet> curSets, List<Curriculum> curriculum) {
    public GreedySolver(int weekNum, double m2, double m3, double m4, double m6, double m7, double F1, double F2, List<CurSet> curSets, List<Curriculum> curriculum) {
        this.weekNum = weekNum;
        schedule = new CurSet[weekNum][25];
        M2 = m2;
        M3 = m3;
        M4 = m4;
//        M5 = m5;
        ...
        startWeek = new int[curSize];    // 记录每门课的开课时间
    ...
    public GreedySolver solve() {
        ...
        for (int origin = 0; origin < weekNum * 25; origin++) {
            px = origin / 25;
            py = origin % 25;
...
//##                    double oldF = calTotalF(schedule, curriculum);
                    this.schedule[tx][ty] = curSetsFor.get(idx2);
                    inserted.add(idx2);
                    this.schedule[tx][ty].setCurSetId(cont++);
                    this.cost += nextF;
//##                    double newF = calTotalF(schedule, curriculum);
//##                    if (nextF != newF - oldF) {
//                        System.out.println("贪心构造初始解过程计算出错");
//                    }
...
//                System.out.println("Successfully insert all nodes");
        ...
        if ((count < n && k == weekNum * 25) || cont != weekNum * 25 + 1) {
