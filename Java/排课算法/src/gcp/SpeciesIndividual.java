...
//        System.out.println("冲突数：" + conflict);
    ...
    public Set<Integer> getConfIndex() {
        ...
        int[] book = new int[NODE_NUM];   //用以记录是否已经被访问
        for (int i = 0; i < NODE_NUM; i++) {
            if (book[i] != 1) {
                Adjacent p = Matrix[i];
                while (p.next != null) {
...
////                    while (confIndex.contains(p.neighbor) && confIndex.contains(i)) {
...
//                    confIndex.add(i);
    ...
    public List<List<Set<Cur>>> getCurSetList(RawInput rawInput, List<SmallCurClass> SmallCurClasses) throws Exception {
        ...
        if (RemainCurs.size() != 0) {
            ...
            List<SpeciesIndividual> smallGenes = SmallGCP(rawInput, RemainCurs, colorNum_max , pc , pm);
