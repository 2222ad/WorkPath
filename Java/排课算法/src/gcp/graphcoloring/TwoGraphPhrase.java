...
public class TwoGraphPhrase {
    ...
    public static List<List<Set<Cur>>> twoGraphColoring(RawInput rawInput ,float pc , float pm) throws Exception {



        // 大图着色
        List<SpeciesIndividual> bigGenes = BigGCP(rawInput , pc , pm);
...
//        }
    ...
    // 大图着色
    private static List<SpeciesIndividual> BigGCP(RawInput rawInput , float pc , float pm) {
        // 决定遗传算法各参数
        GCPData totalGCP = new GCPData(pc , pm);
        ...
        if (rawInput.getCurriculumList().size() <= totalGCP.getColorNum()) {
            int restrictNum = rawInput.getCurriculumList().size() * 2 / 3;
//            int restrictNum = rawInput.getCurriculumList().size() / 2;
        ...
        // 创建遗传算法驱动对象
        GeneticAlgorithm GA = new GeneticAlgorithm(totalGCP);
        ...
        List<SpeciesIndividual> invalidRate = new ArrayList<>();
        if (ConfIndexList.size() != bestRate.size()) {
            System.out.println(" 我真是个智障！");
        }
        for (int k = 0; k < ConfIndexList.size(); k++) {
            ...
            // 决定遗传算法各参数
            GCPData bigGCP = new GCPData(pc , pm);
...
//                System.out.println("#######################################");
                ...
                for (SpeciesIndividual s2 : bestRate2) {
                    ...
                    if (flag) {
                        book = true;
                        for (int m = 0; m < s2.genes.length; m++) {
                            for (int n = m + 1; n < s2.genes.length; n++) {
                                if (Objects.equals(s2.genes[m], s2.genes[n])) {
                                    if (!Collections.disjoint(rawInput.getCurriculumList().get(Integer.parseInt(theCur.get(m).getCurId()) - 1).getClassId(), rawInput.getCurriculumList().get(Integer.parseInt(theCur.get(n).getCurId()) - 1).getClassId()) ||
                                            !Collections.disjoint(rawInput.getCurriculumList().get(Integer.parseInt(theCur.get(m).getCurId()) - 1).getTeacherId(), rawInput.getCurriculumList().get(Integer.parseInt(theCur.get(n).getCurId()) - 1).getTeacherId())) {
                        ...
                        String[] testGenes = bestRate.get(k).genes;
                        for (int m = 0; m < testGenes.length; m++) {
                            for (int n = m + 1; n < testGenes.length; n++) {
                                if (Objects.equals(testGenes[m], testGenes[n])) {
                                    if (!Collections.disjoint(rawInput.getCurriculumList().get(m).getClassId(), rawInput.getCurriculumList().get(n).getClassId()) ||
                                            !Collections.disjoint(rawInput.getCurriculumList().get(m).getTeacherId(), rawInput.getCurriculumList().get(n).getTeacherId())) {
                                        System.out.println(" 图着色过程有误！！！");
                                    }
                                }
                            }
                        ...
                        // 教室类型数量约束
                        Set<Set<Integer>> curSets = bestRate.get(k).geneToSet();
                        for (Set<Integer> curSet : curSets) {
                            Map<Integer, Integer> setRoom = new HashMap<>();
                            for (Integer index : curSet) {
                                int roomKey = rawInput.getCurriculumList().get(index).getRoomType();
                                if (setRoom.containsKey(roomKey)) {
                                    setRoom.put(roomKey, setRoom.get(roomKey) + 1);
                                } else {
                                    setRoom.put(roomKey, 1);
                                }
                            }

                            for (int s = 1; s <= rawInput.getClassroomMap().size(); s++) {
                                if (setRoom.containsKey(s)&&setRoom.get(s) > rawInput.getClassroomMap().get(String.valueOf(s))) {
                                    book = false;
                                    break;
                                }
                            }
                ...
                if (book) {
                    ...
                    invalidRate.add(bestRate.get(k));
                }
            }
...
//            return BigGenes;
//        }
        HashSet<SpeciesIndividual> all = new HashSet<>(bestRate);
        HashSet<SpeciesIndividual> invalid = new HashSet<>(invalidRate);
    ...
    // 小图着色
    public static List<SpeciesIndividual> SmallGCP(RawInput rawInput, List<Cur> RemainCurs, int colorNum_max , float pc , float pm) throws Exception {
        // 决定遗传算法各参数
        GCPData smallGCP = new GCPData( pc , pm);
        smallGCP.Initializing2(rawInput.getCurriculumList(), RemainCurs);   // 将原问题转化为图着色问题
//        smallGCP.setColorNum(colorNum);

        // 创建遗传算法驱动对象
        GeneticAlgorithm GA = new GeneticAlgorithm(smallGCP);
...
//        }
        ...
        for (int i = colorNum_max; i > 0; i--) {
            ...
            List<SpeciesIndividual> bestRate2 = GA.run(speciesPopulation2, smallGCP);
            List<SpeciesIndividual> invalidRate = new ArrayList<>();
            for (int k = 0; k < bestRate2.size(); k++) {
                // 教室类型数量约束
                Set<Set<Integer>> curSets = bestRate2.get(k).geneToSet();
                for (Set<Integer> curSet : curSets) {
                    Map<Integer, Integer> setRoom = new HashMap<>();
                    for (Integer index : curSet) {
                        int roomKey = rawInput.getCurriculumList().get(RemainCurs.get(index).getSupId()).getRoomType();
                        if (setRoom.containsKey(roomKey)) {
                            setRoom.put(roomKey, setRoom.get(roomKey) + 1);
                        } else {
                            setRoom.put(roomKey, 1);
                    ...
                    for (int s = 1; s <= rawInput.getClassroomMap().size(); s++) {
                        if (setRoom.containsKey(s)&&setRoom.get(s) > rawInput.getClassroomMap().get(String.valueOf(s))) {
                            invalidRate.add(bestRate2.get(k));
                            break;
                        }
                    }
                }
            }
            HashSet<SpeciesIndividual> all = new HashSet<>(bestRate2);
            HashSet<SpeciesIndividual> invalid = new HashSet<>(invalidRate);
            all.removeAll(invalid);

            bestRate2 = new ArrayList<>(all);
            if (bestRate2.size() == 0) {
                continue;
            }
